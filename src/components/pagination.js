...
export const initPagination = ({ pages, fromRow, toRow, totalRows }, createPage) => {
    ...
    const applyPagination = (query, state, action) => {
        const limit = state.rowsPerPage; // используем rowsPerPage из state как limit
        let page = state.page; // текущая страница

        // @todo: #2.6 — обработать действия
        if (action) switch (action.name) {
            case 'prev': page = Math.max(1, page - 1); break; // переход на предыдущую страницу
            case 'next': page = Math.min(pageCount || 1, page + 1); break; // переход на следующую страницу, используем pageCount или 1, если pageCount ещё не определён
            case 'first': page = 1; break; // переход на первую страницу
            case 'last': page = pageCount || 1; break; // переход на последнюю страницу, используем pageCount или 1
        }
